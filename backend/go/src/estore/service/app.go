...
func SearchApps(title string, description string) ([]model.App, error) {
	...
	searchResult, err := internal.ESBackend.ReadFromES(query, constants.APP_INDEX)
	if err != nil {
		return nil, err
	}
...
func SaveApp(app *model.App, file multipart.File) error {
	...
	medialink, err := internal.GCSBackend.SaveToGCS(file, app.Id)
	if err != nil {
		return err
	}
	app.Url = medialink

	err = internal.ESBackend.SaveToES(app, constants.APP_INDEX, app.Id)
	if err != nil {
...
func DeleteApp(id string, user string) error {
    ...
    return internal.ESBackend.DeleteFromES(query, constants.APP_INDEX)
}